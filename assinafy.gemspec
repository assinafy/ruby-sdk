# frozen_string_literal: true

require_relative 'lib/assinafy/version'

Gem::Specification.new do |spec|
  spec.name    = 'assinafy'
  spec.version = Assinafy::VERSION
  spec.authors = ['Assinafy SDK Contributors']
  spec.email   = ['sdk@assinafy.com.br']

  spec.summary     = 'Ruby SDK for the Assinafy digital signature API'
  spec.description = 'Ruby SDK for Assinafy. Covers the documented authentication, OAuth 2.1, account, ' \
                     'user, document, signer, assignment, webhook, template, tag, and field APIs, plus the ' \
                     'high-level upload_and_request_signatures helper.'
  spec.homepage    = 'https://github.com/assinafy/ruby-sdk'
  spec.license     = 'MIT'

  spec.required_ruby_version = '>= 3.2'

  spec.metadata['source_code_uri']       = 'https://github.com/assinafy/ruby-sdk'
  spec.metadata['bug_tracker_uri']       = 'https://github.com/assinafy/ruby-sdk/issues'
  spec.metadata['changelog_uri']         = 'https://github.com/assinafy/ruby-sdk/blob/main/CHANGELOG.md'
  spec.metadata['documentation_uri']     = 'https://github.com/assinafy/ruby-sdk/blob/main/docs/API_REFERENCE.md'
  spec.metadata['github_repo']           = 'ssh://github.com/assinafy/ruby-sdk'
  spec.metadata['rubygems_mfa_required'] = 'true'

  spec.files         = Dir['lib/**/*.rb', 'sig/**/*.rbs', 'CHANGELOG.md', 'LICENSE', 'README.md', 'README.en.md'] +
                       ['docs/API_REFERENCE.md']
  spec.require_paths = ['lib']

  spec.add_dependency 'faraday', '>= 2.14.3', '< 3.0'
  spec.add_dependency 'faraday-multipart', '>= 1.0', '< 2.0'

  spec.add_development_dependency 'bundler-audit', '~> 0.9'
  spec.add_development_dependency 'rake', '~> 13.0'
  spec.add_development_dependency 'rspec', '~> 3.0'
  spec.add_development_dependency 'rubocop', '~> 1.86'
  spec.add_development_dependency 'rubocop-rspec', '~> 3.6'
  spec.add_development_dependency 'webmock', '~> 3.0'
end
